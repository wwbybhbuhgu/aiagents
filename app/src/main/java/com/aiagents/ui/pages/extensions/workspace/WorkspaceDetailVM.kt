...
class WorkspaceDetailVM(
    ...
    init {
        loadWorkspace()
        refresh()
        // 容器必选: 进入页面时若 rootfs 未安装/未就绪, 强制自动下载（无需用户手动选择）
        viewModelScope.launch {
            val ws = repository.getById(id) ?: return@launch
            val status = ws.shellStatus
            if (status != WorkspaceShellStatus.READY.name && status != WorkspaceShellStatus.INSTALLING.name) {
                installRootfs(DEFAULT_ROOTFS_URL)
            }
        }
    }
