...
fun ASRProviderConfigure(
    ...
    Column(
        ...
        when (setting) {
            ...
            is ASRProviderSetting.LocalStreamingSTT -> LocalSTTConfiguration(setting, onValueChange)
        }
    }
}

@Composable
private fun LocalSTTConfiguration(
    setting: ASRProviderSetting.LocalStreamingSTT,
    onValueChange: (ASRProviderSetting) -> Unit
) {
    FormItem(
        label = { Text("采样率") },
        description = { Text("本地模型固定使用 16kHz") }
    ) {
        OutlinedTextField(
    ...
    FormItem(
        label = { Text("模型位置") },
        description = { Text("模型文件打包在 APK assets 中, 无需额外配置") }
    ) {
        OutlinedTextField(
            value = "sherpa/models (内置)",
            onValueChange = {},
            readOnly = true,
            modifier = Modifier.fillMaxWidth(),
        )
