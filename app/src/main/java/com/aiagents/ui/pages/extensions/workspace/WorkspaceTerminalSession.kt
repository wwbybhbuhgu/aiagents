...
import com.aiagents.workspace.RootfsPatchOptions
import com.aiagents.workspace.RootfsPatcher
import java.io.File
...
internal fun createWorkspaceTerminalSession(
    ...
    val skillsDir = File(Environment.getExternalStorageDirectory(), "$AI_AGENT_SHARED_DIR/${FileFolders.SKILLS}").apply { mkdirs() }
    ...
    val args = mutableListOf(
        ...
        "-b",
        "${File(appContext.filesDir, FileFolders.UPLOAD).apply { mkdirs() }.absolutePath}:$UPLOAD_DIR",
        "-b",
        "${File(Environment.getExternalStorageDirectory(), "$AI_AGENT_SHARED_DIR/${FileFolders.SD_DIR}").apply { mkdirs() }.absolutePath}:$SD_DIR",
    )
...
internal fun prepareWorkspaceTerminalSession(context: Context, root: String) {
    ...
    File(Environment.getExternalStorageDirectory(), "$AI_AGENT_SHARED_DIR/${FileFolders.SKILLS}").mkdirs()
    RootfsPatcher().patch(
        linuxDir,
