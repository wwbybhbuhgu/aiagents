...
private fun buildWorkspacePrompt(workspace: WorkspaceEntity, cwd: String? = null): String = buildString {
    ...
    appendLine("- Inline file display: markdown can ONLY render IMAGES inline — it CANNOT play audio, play video, or show a text/document file. So: (a) to show an IMAGE inline, call `show_file` with its container path to get a `content://` uri, then embed it in markdown WITHOUT angle brackets: `![desc](content://<app>.workspacefile/${workspace.id}/workspace/xxx.png)` (do NOT wrap the uri in `<` and `>`). (b) For EVERYTHING else — audio, video, pdf, text/markdown documents, zip, etc. — you must use the `show_file` tool directly (it renders them as a proper built-in media/document card with a player/viewer). NEVER paste a text document's contents or a non-image file's path into markdown; markdown cannot display it. This is exactly why `show_file` exists: it extends display beyond what markdown can render.")
