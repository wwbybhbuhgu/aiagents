...
private fun buildWorkspacePrompt(workspace: WorkspaceEntity, cwd: String? = null): String = buildString {
    ...
    appendLine("- Internal file protocol: to display a file (image/video/audio/text) directly in your reply, use `aiagents-file://${workspace.id}<container-absolute-path>` in markdown. The container-absolute-path is the full path inside this workspace's Rootfs, starting from the workspace root `/workspace` (e.g. `![img](aiagents-file://${workspace.id}/workspace/images/xxx.png)`). The `<workspaceId>` part is ALWAYS your current workspace id `${workspace.id}`; just paste it and append the absolute path verbatim. The app resolves this protocol to show the file inline, no conversion needed.")
