...
private fun buildWorkspacePrompt(workspace: WorkspaceEntity, cwd: String? = null): String = buildString {
    ...
    appendLine("- Inline file display: to show a workspace file (image/video/audio) directly in your reply, call the `show_file` tool with the file's container absolute path (e.g. `/workspace/images/xxx.png`). It returns a `content://` uri (Android native file protocol) whose path is the full container path, e.g. `content://<app>.workspacefile/${workspace.id}/workspace/images/xxx.png`. Embed that uri verbatim in markdown: `![desc](<uri>)`. The app serves the file from the workspace rootfs via a ContentProvider; this is the only supported protocol for inline files — do not invent other schemes.")
