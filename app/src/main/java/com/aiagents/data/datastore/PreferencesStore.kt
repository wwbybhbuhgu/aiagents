package com.aiagents.data.datastore
...
import com.aiagents.data.ai.mcp.McpServerConfig
import com.aiagents.data.ai.prompts.DEFAULT_COMPRESS_PROMPT
import com.aiagents.data.ai.prompts.DEFAULT_OCR_PROMPT
import com.aiagents.data.ai.prompts.DEFAULT_SUGGESTION_PROMPT
import com.aiagents.data.ai.prompts.DEFAULT_TITLE_PROMPT
import com.aiagents.data.ai.prompts.LEARNING_MODE_PROMPT
import com.aiagents.asr.ASRProviderSetting
import com.aiagents.data.datastore.migration.PreferenceStoreV1Migration
import com.aiagents.data.datastore.migration.PreferenceStoreV2Migration
...
import com.aiagents.data.model.QuickMessage
import com.aiagents.data.model.Tag
import com.aiagents.data.sync.s3.S3Config
import com.aiagents.ui.theme.CustomTheme
import com.aiagents.ui.theme.PresetThemes
import com.aiagents.utils.JsonInstant
import com.aiagents.utils.toMutableStateFlow
import com.aiagents.search.SearchCommonOptions
    ...
    val settingsFlowRaw = dataStore.data
        .catch { exception ->
            ...
            Settings(
                ...
                assistants = JsonInstant.decodeFromString<List<Assistant>>(preferences[ASSISTANTS] ?: "[]").let { list ->
                    // 迁移: 旧数据没有 enablePhoneAutomation 字段时, Agent 模式默认开启手机自动化;
                    // 一旦字段已写入(用户保存过), 则尊重已保存的值
                    if (preferences[ASSISTANTS]?.contains("\"enablePhoneAutomation\"") != true) {
                        list.map { it.copy(enablePhoneAutomation = true) }
                    } else {
                        list
                    }
                }.let { list ->
                    // 迁移: 旧数据没有 enableBrowserAutomation 字段时, 默认开启浏览器自动化
                    if (preferences[ASSISTANTS]?.contains("\"enableBrowserAutomation\"") != true) {
                        list.map { it.copy(enableBrowserAutomation = true) }
                    } else {
                        list
                    }
                },
...
data class DisplaySetting(
    ...
    val floatingAsrSilenceSeconds: Int = 3,
)
...
data class WebDavConfig(
    ...
    val username: String = "",
    val password: String = "",
    val path: String = "aiagents_backups",
...
internal val DEFAULT_ASSISTANTS = listOf(
    ...
    Assistant(
        ...
        name = "",
        systemPrompt = """
            You are a helpful AI assistant, called {{char}}, based on model {{model_name}}.

            You proactively use the available tools to help the user, instead of just replying with text.
            When the user asks for something the tools can do, call the right tool, examine the result,
            and give a clear, concise answer. Prefer doing over describing.
            ...
            - Use Markdown for formatting and LaTeX for math.
            - For images: if the user asks what is in a picture, asks about the content of an image, or asks to
              solve a problem that may live inside an image, always analyze the actual image with the image
              analysis tool (`image_analysis`) rather than guessing or relying only on OCR text. Only use OCR
              when the user specifically wants to extract text verbatim. If you are unsure whether image
              analysis is needed, still analyze the image first.
        """.trimIndent()
    ),
)
