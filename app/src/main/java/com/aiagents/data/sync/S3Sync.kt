package com.aiagents.data.sync
...
import com.aiagents.data.datastore.migration.SettingsJsonMigrator
import com.aiagents.data.sync.s3.S3Client
...
class S3Sync(
    ...
    suspend fun backupToS3(config: S3Config) = withContext(Dispatchers.IO) {
        ...
        val client = getS3Client(config)
        val key = "aiagents_backups/${file.name}"
    ...
    suspend fun listBackupFiles(config: S3Config): List<S3BackupItem> = withContext(Dispatchers.IO) {
        ...
        val result = client.listObjects(
            prefix = "aiagents_backups/",
            maxKeys = 1000
        ).getOrThrow()

        result.objects
            .filter { it.key.startsWith("aiagents_backups/backup_") && it.key.endsWith(".zip") }
            .map { obj ->
                S3BackupItem(
                    key = obj.key,
    ...
    suspend fun prepareBackupFile(config: S3Config): File = withContext(Dispatchers.IO) {
        ...
        ZipOutputStream(FileOutputStream(backupFile)).use { zipOut ->
            ...
            if (config.items.contains(S3Config.BackupItem.DATABASE)) {
                val dbFile = context.getDatabasePath("ai_agents")
                if (dbFile.exists()) {
                    addFileToZip(zipOut, dbFile, "ai_agents.db")
                }

                val walFile = File(dbFile.parentFile, "ai_agents-wal")
                if (walFile.exists()) {
                    addFileToZip(zipOut, walFile, "ai_agents-wal")
                }

                val shmFile = File(dbFile.parentFile, "ai_agents-shm")
                if (shmFile.exists()) {
                    addFileToZip(zipOut, shmFile, "ai_agents-shm")
                }
            }
            ...
            while (zipIn.nextEntry.also { entry = it } != null) {
                entry?.let { zipEntry ->
                    ...
                    when (zipEntry.name) {
                        ...
                        "ai_agents.db", "ai_agents-wal", "ai_agents-shm" -> {
                            if (config.items.contains(S3Config.BackupItem.DATABASE)) {
                                val dbFile = when (zipEntry.name) {
                                    "ai_agents.db" -> context.getDatabasePath("ai_agents")
                                    "ai_agents-wal" -> File(
                                        context.getDatabasePath("ai_agents").parentFile,
                                        "ai_agents-wal"
                                    )
