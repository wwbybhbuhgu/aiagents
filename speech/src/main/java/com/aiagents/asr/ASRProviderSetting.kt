...
sealed class ASRProviderSetting {
    ...
    data class Step(
        ...
        // 是否返回词级时间戳 (aiagents 当前不消费时间戳, 默认关闭节省 token)
    ...
    @Serializable
    @SerialName("local_streaming_stt")
    data class LocalStreamingSTT(
        override val id: Uuid = Uuid.random(),
        override val name: String = "本地实时语音转文本",
        val sampleRate: Int = 16000,
    ) : ASRProviderSetting() {
        override fun copyProvider(
            id: Uuid,
