...
class RootfsPatcher {
    fun patch(
        ...
        ensureBindMountPoints(linuxDir)
    }
    ...
    private fun ensureRootfsDns(
        ...
        resolvConf.writeText(
            buildString {
                appendLine("# Generated by AIAgents workspace.")
    ...
    private fun ensureBindMountPoints(linuxDir: File) {
        // proot-droid(--link2symlink) 对不存在的 guest 挂载点用符号链接伪装,
        // 在某些文件系统上创建失败会导致 -b 绑定静默失效;
        // 提前在 rootfs 里建好空挂载点目录, 保证容器内始终能看到这些目录及其内容
        listOf(
            "upload", "tool_outputs", "skills", "memories", "screenshots", "sd", "workspace",
        ).forEach { name ->
            File(linuxDir, name).apply {
                mkdirs()
