 ...
 */
fun List<UIMessage>.limitContext(limit: Int, preserveFirst: Int = 0): List<UIMessage> {
    if (limit <= 0 || this.size <= limit) return this

    // 受保护头部(如角色开场白)不计入上限, 始终保留; 从剩余消息里截断
    val preserve = preserveFirst.coerceIn(0, this.size - 1)
    val protectedHead = this.take(preserve)
    val rest = this.drop(preserve)
    ...
    val startIndex = (((rest.size - limit) / stride + 1) * stride).coerceAtMost(rest.size - 1)

    return protectedHead + rest.subList(alignContextStart(startIndex), rest.size)
}
